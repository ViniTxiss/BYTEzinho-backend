...
def iniciar_gspread() -> Optional[gspread.Worksheet]:
    """Autentica com a API do Google e abre a planilha de leads."""
    sheet_name = os.getenv("GOOGLE_SHEET_NAME")
    credentials_file = os.getenv("GOOGLE_CREDENTIALS_FILE")

    if not sheet_name or not credentials_file:
        logging.error("❌ ERRO: Variáveis de ambiente para o Google Sheets não configuradas.")
        return None
    try:
        # Usa a conta de serviço para autenticar
        gc = gspread.service_account(filename=credentials_file)
        spreadsheet = gc.open(sheet_name)
        return spreadsheet.sheet1 # Retorna a primeira aba da planilha
    except Exception as e:
        logging.error(f"❌ ERRO: Falha ao conectar com o Google Sheets: {e}")
        return None
...
# Carrega as origens permitidas a partir de uma variável de ambiente.
# No seu arquivo .env, você pode ter algo como:
# ALLOWED_ORIGINS="http://localhost:3000,http://127.0.0.1:5500,https://seu-frontend.vercel.app"
allowed_origins_str = os.getenv("ALLOWED_ORIGINS", "")
origins = [origin.strip() for origin in allowed_origins_str.split(",") if origin]

# Adiciona origens padrão de desenvolvimento se nenhuma for especificada no ambiente
if not origins:
...
def startup_event():
    ...
    if not worksheet:
        logging.warning("[AVISO] A integração com Google Sheets falhou. Os leads não serão salvos.")
    else:
...
async def capturar_lead(lead: Lead):
    ...
    if not worksheet:
        raise HTTPException(status_code=503, detail="Serviço de armazenamento de leads indisponível.")

    logging.info(f"✅ Novo lead recebido: Nome='{lead.name}', Email='{lead.email}'")
    try:
        # Adiciona uma nova linha na planilha com os dados do lead
        worksheet.append_row([lead.name, lead.email])
        return {"status": "success", "message": "Lead recebido com sucesso!"}
    except Exception as e:
        logging.error(f"❌ ERRO ao salvar lead no Google Sheets: {e}")
